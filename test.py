...
from tools.tools import MongoDBUtility, Googletoolkit  # Assuming your tools are in tools/tools.py
...
# --- Streamlit App ---
st.title("MongoDB Query Agent UI")

# Initialize MongoDB client and agent
@st.cache_resource
def create_agent():
    """Creates and caches the Agno agent."""
    mongodb_utility = MongoDBUtility()
    google_toolkit = Googletoolkit(
        credentials_path=os.getenv("credentials_path"),
        sheet_id=os.getenv("sheet_id"),
    )
    ...
    agent = Agent(
        ...
        tools=[mongodb_utility, ThinkingTools(), google_toolkit, PandasTools()],
        instructions="""
...
agent = create_agent()  # Initialize the agent

# Text input for prompt
prompt = st.text_input("Enter your MongoDB query:")

# Generate response if prompt is given
if prompt:
